...
void M2MBase::set_interface_description(const String &desc)
{
    if (!_is_static) {
        ...
        _sn_resource->dynamic_resource_params->static_resource_parameters->interface_description_ptr =
                alloc_string_copy((uint8_t*)desc.c_str(), desc.length());
...
void M2MBase::set_resource_type(const String &res_type)
{
    if (!_is_static) {
        ...
        _sn_resource->dynamic_resource_params->static_resource_parameters->resource_type_ptr =
                alloc_string_copy((uint8_t*)res_type.c_str(), res_type.length());
...
{
    return _sn_resource->dynamic_resource_params->static_resource_parameters;
}
...
const String& M2MBase::name() const
{
    ...
    if (NULL != _sn_resource->name) {
        ret = ((const char*)_sn_resource->name);
    }
    return ret;
}
...
{
    String ret = "";
    if (NULL!=_sn_resource->dynamic_resource_params->static_resource_parameters->interface_description_ptr) {
         ret = String((const char*)_sn_resource->dynamic_resource_params->static_resource_parameters->interface_description_ptr);
    }
    return ret;
}
...
{
    String ret = "";
    if (NULL!=_sn_resource->dynamic_resource_params->static_resource_parameters->resource_type_ptr) {
         ret = String((const char*)_sn_resource->dynamic_resource_params->static_resource_parameters->resource_type_ptr);
    }
    return ret;
}
...
void M2MBase::set_uri_path(const String &path)
{
    if (!_is_static) {
        ...
        _sn_resource->dynamic_resource_params->static_resource_parameters->path =
                alloc_string_copy((uint8_t*)path.c_str(), path.length());
        _sn_resource->dynamic_resource_params->static_resource_parameters->pathlen =
                path.length();
    }
}
...
{
    if(_sn_resource->dynamic_resource_params->static_resource_parameters->path != NULL) {
        return String((const char*)_sn_resource->dynamic_resource_params->static_resource_parameters->path);
        }
    else {
        return String("");
    }
